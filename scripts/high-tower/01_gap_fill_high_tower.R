...
ec_in <- readRDS('data/high-tower/ec_high_tower_30_min_2021_2023_qc_rough.rds')

findGaps(ec_in, gapfile = 'crhm-logs/high-tower/ec_in_gaps.csv')
...
le_still_have_gaps <- findGaps(le_fill, gapfile = 'crhm-logs/high-tower/le_gaps.csv', quiet = F)
...
h_still_have_gaps <- findGaps(h_fill, gapfile = 'crhm-logs/high-tower/h_gaps.csv', quiet = F)
...
u_star_still_have_gaps <- findGaps(u_star_fill, gapfile = 'crhm-logs/high-tower/u_star_gaps.csv', quiet = F)
...
tau_still_have_gaps <- findGaps(tau_fill, gapfile = 'crhm-logs/high-tower/tau_gaps.csv', quiet = F)
...
saveRDS(ec_fill_out, 'data/high-tower/ec_high_tower_30_min_2021_2023_qc_rough_semifill.rds')
...
ec_fill_out_daily <- ec_fill_out |> 
  ...
  mutate(value = case_when(
    count_nans > (0.25 * (24*2)) ~ NA, # check if have more than 90% of measurements
    TRUE ~ value
  ))|> 
  select(-count_nans) |>
  pivot_wider()

saveRDS(ec_fill_out_daily, 'data/high-tower/ec_high_tower_2021_2023_qc_rough_semifill_daily.rds')
